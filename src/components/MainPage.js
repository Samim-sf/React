...
class AddNewTask extends Component {
    ...
    addTasks = () => {
        ...
        const desc = (ReactDOM.findDOMNode(this.refs.description)).value;
        console.log(title);
        console.log(desc);
        const task = new Task(title, desc);
        this.setState(task);
        console.log(this.state.title);
        // <AddCards task={this.state}/>
        console.log("2");
        ...
        return (
            <div className={"d-flex flex-column container justify-content-center align-content-center"}>
                ...
                <form className={"d-flex flex-column container justify-content-center align-content-center"}>
                    <input name="title" useRef="title" className={"m-3 bg-light"} type="text " placeholder={"Title"}/>
                    <textarea ref="description" className={"m-3  bg-light"} name="description" cols="30" rows="10"
                              placeholder={"Description"}/>
                    <button className={"m-3 btn-primary"} type="button" onClick={this.addTasks}>+ Add</button>
                </form>
            </div>
